...
- `analysis.stall.lane_beats(beats, readings, interval)` turns a lane's readings into beats every rule reads CPU from. It is used for a lane's row (`topology`) and its stall verdict (`incidents/stall.py`). A lane with fewer than two readings, or whose readings stopped while the beats went on (its thread has ended), has none: its `cpu_rate` is null and never its process's, which read an idle lane beside a busy one as `working` (S2). A stall assessed before a lane's second reading waits for the next poll rather than read "no figure" as blocked.
...
- A lane row's `cpu_rate` is null where the lane has no readings of its own, and a lane with no test in flight says so in `why`.
