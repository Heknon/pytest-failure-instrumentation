...
#: A suite whose fixtures are *generated*: one factory, one scope callable
#: answering for all of them, plus a parametrized one and one whose params
#: come from ``pytest_generate_tests``. Every setup and teardown is written to
#: a file with the process that ran it, so the sequence a run produced can be
#: compared with the sequence another run produced. The value the log holds is
#: not what pytest does with these fixtures - that is pytest's business, and it
#: changes between versions - but that it is the *same* with the plugin on.
#:
#: The three ways a session fixture is legitimately set up more than once are
#: all in here, because each is the kind of thing a reporting plugin could
#: disturb by wrapping the runtest protocol: a parametrized fixture is cached
#: one instance at a time, so a change of param between consecutive tests tears
#: it down; ``pytest_generate_tests`` parametrizes at a scope of its own that
#: outranks the fixture's for ordering; and ``getfixturevalue`` asks at run
#: time for what collection never saw.
GENERATED_FIXTURES_CONFTEST = INNER_CONFTEST + '''
...
def record(what):
    ...
    with open("fixtures.log", "a", encoding="utf-8") as handle:
        handle.write(f"{worker} {what}\\n")


def scope_of(*, fixture_name, config):
    """The scope is in the name: made_session, made_module, made_function."""
    return fixture_name.rsplit("_", 1)[-1]


def generate(name):
    @pytest.fixture(scope=scope_of, name=name)
    def made(request):
        param = getattr(request, "param", "-")
        record(f"setup {name} {param}")
        yield name
        record(f"teardown {name} {param}")

    return made


for _name in ("made_session", "made_module", "made_function"):
    globals()[_name] = generate(_name)


@pytest.fixture(scope=scope_of, params=["x", "y"], name="params_session")
def params_session(request):
    record(f"setup params_session {request.param}")
    yield request.param
    record(f"teardown params_session {request.param}")


@pytest.fixture(scope=scope_of, name="gen_session")
def gen_session(request):
    record(f"setup gen_session {request.param['k']}")
    yield request.param
    record(f"teardown gen_session {request.param['k']}")


def pytest_generate_tests(metafunc):
    if "gen_session" in metafunc.fixturenames:
        metafunc.parametrize(
            "gen_session", [{"k": 1}, {"k": 2}], indirect=True, scope="module", ids=["g1", "g2"]
        )
'''

GENERATED_FIXTURES_SUITE = {
    "test_first": """
def test_one(made_session, made_module, made_function, params_session, gen_session):
    assert made_session == "made_session"


def test_two(made_session, made_module, request):
    assert request.getfixturevalue("made_function") == "made_function"
""",
    "test_second": """
def test_three(made_session, made_module, made_function, gen_session):
    assert gen_session["k"] in (1, 2)


def test_four(made_session, params_session):
    assert params_session in ("x", "y")
""",
}


def _fixture_sequence(pytester: pytest.Pytester, *arguments: str, passed: int) -> list[str]:
    """Run the generated-fixture suite and return what its fixtures recorded,
    as one sequence per process, in a process-independent order."""
    log = pytester.path / "fixtures.log"
    if log.exists():
        log.unlink()
    result = pytester.runpytest_subprocess("-p", "no:cacheprovider", *arguments, timeout=180)
    result.assert_outcomes(passed=passed)
    by_process: dict[str, list[str]] = {}
    for line in log.read_text(encoding="utf-8").splitlines():
        process, _, what = line.partition(" ")
        by_process.setdefault(process, []).append(what)
    return sorted("\n".join(sequence) for sequence in by_process.values())


@pytest.mark.parametrize(
    "switch",
    [
        pytest.param([ENABLE_FLAG], id="instrumentation"),
        pytest.param(["--failure-profile"], id="profile"),
        pytest.param([ENABLE_FLAG, "--callstack-port", "0"], id="live-view"),
    ],
)
def test_generated_dynamic_scope_fixtures_are_cached_exactly_as_without_the_plugin(
    runner, switch
):
    """Wrapping every phase of the runtest protocol, and the protocol itself,
    must not change when pytest sets a fixture up or tears it down.

    Fixtures generated by a factory with a scope decided by a callable are
    the case that has to be checked rather than assumed: their scope is
    evaluated once, when pytest walks the conftest, and everything that
    follows - which node the value is cached on, when a parametrized one is
    torn down for the next param, what ``pytest_generate_tests`` does to the
    order - is decided from that. A plugin that touched any of it would
    show up here as a setup that happened in one run and not the other.
    """
    runner.pytester.makeconftest(GENERATED_FIXTURES_CONFTEST)
    runner.pytester.makepyfile(**GENERATED_FIXTURES_SUITE)
    # 2 params of gen_session x 2 of params_session for test_one, one
    # test_two, 2 for test_three, 2 for test_four. The baseline is the plugin
    # installed and not switched on - its options and hookspecs on the run,
    # nothing registered - which is what a run without the switch is.
    without = _fixture_sequence(runner.pytester, passed=9)

    # The baseline has to be the thing it claims to be before it is compared
    # against: one process, and a session fixture set up once in it.
    assert len(without) == 1
    (sequence,) = without
    assert sequence.count("setup made_session -") == 1
    assert sequence.count("setup made_function -") == 7, "one per test that asks"
    # At least once per module. pytest orders the run by the session-scoped
    # params first, which interleaves the two modules and sets a module
    # fixture up again each time it comes back to one - and keeps one
    # instance of a parametrized fixture at a time, whatever its scope. Both
    # are pytest's to decide, on this version; the plugin's job is to leave
    # whatever it decided alone.
    assert sequence.count("setup made_module -") >= 2
    assert sequence.count("setup params_session x") >= 1

    with_plugin = _fixture_sequence(runner.pytester, *switch, passed=9)
    assert with_plugin == without


@needs_xdist
def test_generated_dynamic_scope_fixtures_are_cached_exactly_as_without_the_plugin_on_workers(
    runner,
):
    """The same, in the processes that record: under xdist a worker wraps the
    protocol and the controller does not, and ``--dist loadfile`` hands each
    worker one whole module, so each worker's sequence is a module's."""
    runner.pytester.makeconftest(GENERATED_FIXTURES_CONFTEST)
    runner.pytester.makepyfile(**GENERATED_FIXTURES_SUITE)
    distribution = ("-n", "2", "--dist", "loadfile")

    without = _fixture_sequence(runner.pytester, *distribution, passed=9)
    assert len(without) == 2, "each of the two modules on a worker of its own"
    assert all(sequence.count("setup made_session -") == 1 for sequence in without)

    with_plugin = _fixture_sequence(runner.pytester, ENABLE_FLAG, *distribution, passed=9)
    assert with_plugin == without
